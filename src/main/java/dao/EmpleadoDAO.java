...
public class EmpleadoDAO {
    ...
    private void notificarObservers(Empleado empleado) {
        for (Observer observer : observers) {
            observer.actualizar(empleado);
        }
    }
	 ...
	 * @return El objeto Empleado correspondiente si se encuentra, de lo contrario,
	 *         null.
	 */
	...
	public void actualizarEmpleado(Empleado empleado) {
		...
		try (Connection connection = DatabaseConnection.getConnection();
			...
			// Notificar a los observadores después de actualizar al empleado
	        notificarObservers(empleado);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * Elimina un empleado de la base de datos por su DNI. También elimina la nómina
	...
	/**
	 * Agrega un nuevo empleado a la base de datos. También inserta una nómina para
	 * el nuevo empleado.
	 * 
	 * @param empleado El objeto Empleado que contiene la información del nuevo
	 *                 empleado.
	 */
	 ...
	 * @param nombre    Una cadena que el nombre del empleado debe contener
	 *                  (opcional).
	...
	public List<Empleado> filtrarEmpleados(String sexo, String nombre, Integer categoria, Integer anyos) {
		...
		return empleados.stream().filter(e -> (sexo == null || sexo.isEmpty() || e.getSexo().equals(sexo))) // Filtra
																											// por sexo
				.filter(e -> (nombre == null || nombre.isEmpty()
						|| e.getNombre().toLowerCase().contains(nombre.toLowerCase()))) // Filtra por nombre
